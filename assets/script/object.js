...
class Celestial_Object {
  ...
  calcDistance(posX, posY) {
    return (
      Math.sqrt(Math.pow(posX - this.posX, 2) + Math.pow(posY - this.posY, 2)) *
      1000000
    ); // magic number?
  }
  ...
  set90Angle(object) {
    this.direction =
      this.calcDirection(object.posX, object.posY) - this.direction - 90;
  }
  applyForces(object) {
  ...
  detectCollision(object, expand = 0) {
    ...
    return distance <= this.radius + object.radius + expand;
  }
  displayForces(object, ctx) {
    ...
    ctx.beginPath();
    ctx.arc(B["x"], B["y"], this.radius, 0, 2 * Math.PI);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.arc(D["x"], D["y"], this.radius, 0, 2 * Math.PI);
    ctx.fill();
